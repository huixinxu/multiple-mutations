...
def main(args):
    ...
    multi_hits = process_overlaps(args.multi)
    probs = get_mut_probs(args.probs)

    labels = ['#gene', 'mutations', '#LoF', '#mis', 'prob(LoF)', 'prob(mis)',
              'prob(LoF+mis)', '2*prob', 'exp#[{0}]'.format(args.num), 'ppois',
              'compared_to']
    print('\t'.join(labels))
    ...
    for gene in multi_hits.keys():
        ...
        results.extend([plof, pmis, plofmis])

        if results[3] > 0:
            results = determine_significance(results, plofmis, args.num, 'lofmis')
        ...
        if results[2] > 1:
            if results[-1] == 'LoF+mis':
                results = results[:-4]
            l_results = determine_significance(results, plof, args.num, 'lof')
            l_results.append('LoF')
            print('\t'.join(map(str, l_results)))
